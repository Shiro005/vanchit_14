...
const FamilyManagement = ({ voter, onUpdate, candidateInfo }) => {
  ...
  // 🔸 Enhanced search function with transliteration, fuzzy matching, and multi-field search
  ...
  // 🔸 Enhanced Search + pagination with debouncing
  useEffect(() => {
  ...
  }, [modalQuery, availableVoters, currentVoterSurname, voterData]);
  ...
  const generateFamilyWhatsAppMessage = () => {
    ...
    message += `यादी भाग क्रमांक: ${(`${voterData.ac1}/${voterData.ac2}/${voterData.ac3}`)}\n`;
    ...
    familyMembers.forEach((member, index) => {
      ...
      message += `यादी भाग क्रमांक: ${(`${member.ac1 || ''}/${member.ac2 || ''}/${member.ac3 || ''}`)}\n`;
    ...
    message += `${candidateInfo.messageWhatsapp}`;
    message += `${WEBSITE_URL}`;
          ...
          <div className="bg-white rounded-2xl w-full max-w-4xl max-h-full overflow-hidden flex flex-col shadow-2xl">
            ...
            <div className="p-4 border-b relative bg-white">
              ...
              <div className="text-xs text-gray-500 mt-2 flex justify-between">
                <span>
                  <TranslatedText>Search works with: Marathi name, English name, Voter ID, Serial Number</TranslatedText>
                </span>
                <span>
                  <TranslatedText>Found: {filteredVoters.length} voters</TranslatedText>
                </span>
              </div>
            </div>
              ...
              {paginatedVoters.length > 0 ? (
                ...
                <div className="text-center text-gray-500 py-12">
                  ...
                  <p className="text-sm mt-2 max-w-md mx-auto">
                    <TranslatedText>Try searching by Marathi name, English name, Voter ID, or Serial Number. The system supports transliteration for cross-language searches.</TranslatedText>
                  </p>
                  {modalQuery.trim() && (
                    <div className="mt-4 p-3 bg-gray-100 rounded-lg inline-block">
                      <p className="text-sm">Search query: "{modalQuery}"</p>
                    </div>
                  )}
                </div>
              )}
            ...
            <div className="p-4 border-t border-gray-200 flex justify-between items-center text-sm text-gray-600 bg-white">
              <div>
                <TranslatedText>Showing {Math.min(pageSize, paginatedVoters.length)} of {filteredVoters.length} voters</TranslatedText>
              </div>
              <div className="flex items-center gap-2">
                <button
